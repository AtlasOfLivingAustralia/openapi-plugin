...
@Slf4j
class GrailsOpenApiReader implements OpenApiReader {
...
//    private Application application
    ...
    GrailsOpenApiReader(OpenAPI openAPI) {
        this()
        setConfiguration(new SwaggerConfiguration().openAPI(openAPI))
    }

    GrailsOpenApiReader(OpenAPIConfiguration openApiConfiguration) {
        this()
     ...
     */
    OpenAPI read(Set<Class<?>> classes) {
...
//        def responsesFormats = cls.getField('responseFormats').get(null)
        ...
        // TODO Remove? We're expecting all params to appear in @Operation
...
//        globalParameters.addAll(ReaderUtils.collectFieldParameters(cls, components, classConsumes, null))
        ...
        for (Method method : methods) {
            ...
            String controllerPath = apiPath?.value() ?: jaxRsApiPath?.value()
            String actionPath = methodPath?.value() ?: jaxRsMethodPath?.value()
            String operationPath = getPath(
                    actionPath ? controllerPath : null,
                    actionPath ?: linkGenPath,
                    parentPath, isSubresource)
//            if (apiPath || methodPath) {
//                operationPath = ReaderUtils.getPath(apiPath, methodPath, parentPath, isSubresource);
//            } else {
//                operationPath = methodPath ? methodPath.value() : methodJaxRsPath ? methodJaxRsPath.value() : linkGenPath
//            }
            ...
            if (operationPath != null) {
                ...
                // TODO Does grails need anything like JAX-RS sub resources?
...
//                }
                ...
                String httpMethod = apiOperation.method()?.toLowerCase()
...
//                null,
     ...
     * @param classLevelPath
     * @param methodLevelPath
     * @param parentPath
     * @param isSubresource
     * @return
     */
    private static String getPath(String classLevelPath, String methodLevelPath, String parentPath, boolean isSubresource) {
